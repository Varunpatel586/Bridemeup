  ...
  if (loading || dataLoading) {
    return (
      <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center">
        <p className="text-[#C5A880] tracking-widest uppercase text-sm animate-pulse">Loading Dashboard...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-[#1A1A1A] font-sans flex flex-col">
      ...
      <main className="flex-1 pt-24 pb-16 px-6 max-w-7xl mx-auto w-full">
        <div className="mb-12">
            ...
            <p className="text-[#1A1A1A]/60">Manage your upcoming bridal appointments and view your saved neural wedding plans.</p>
        </div>
                ...
                {appointments.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-4">
                        {appointments.map((apt) => (
                            <div key={apt.id} className="bg-white border border-neutral-100 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 className="font-medium text-lg">{apt.salons?.name}</h3>
                                        <p className="text-sm text-[#1A1A1A]/60">{apt.salons?.address}</p>
                                    </div>
                                    <span className={`text-xs px-3 py-1 rounded-full uppercase tracking-widest ${apt.status === 'pending' ? 'bg-orange-50 text-orange-700' : 'bg-green-50 text-green-700'}`}>
                                        {apt.status}
                                    </span>
                                </div>
                                <div className="flex flex-col gap-2 text-sm">
                                    ...
                                    <div className="flex items-center gap-2 text-[#1A1A1A]/70">
                                        <span>Stylist: <strong className="text-[#1A1A1A]">{apt.stylists?.name}</strong> ({apt.stylists?.role})</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                {weddingPlans.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-4">
                        {weddingPlans.map((plan) => (
                            <div key={plan.id} className="bg-white border border-neutral-100 rounded-xl p-6 shadow-sm">
                                <p className="text-xs text-[#1A1A1A]/40 mb-3">Saved on {new Date(plan.created_at).toLocaleDateString()}</p>
